...
(async () => {
  ...
  const B='formFiltros:j_idt115';
  const { pg } = await attach();
  await goto(pg, '/pages/reportePlanCuota'); await pg.waitForTimeout(4000);
  await pg.click(`[id="${B}:botonLimpiar"]`).catch(()=>{}); await pg.waitForTimeout(4500);
  console.log('rdv : (Plan vs Cuota NO tiene combo de vendedor)');
  console.log('clas:', await clickPick(pg, B+':clasificacion', clas));
  const marcados = await pg.evaluate((n)=>{
    const w=PrimeFaces.widgets['widget_formFiltros_j_idt115_checkboxValor'];
  ...
  const pre = await pg.evaluate((b)=>{const g=s=>{const e=document.getElementById(s);return e?e.value:null};
    return {clas:g(b+':clasificacion_input'),cumpl:g(b+':cumplimiento_input'),
  ...
  const filas = await pg.evaluate(()=>{const t=document.getElementById('formTabla:tablaComparativoPlanCuota');
    return t? [...t.querySelectorAll('tbody tr')].map(tr=>[...tr.querySelectorAll('td')].map(td=>td.innerText.trim().replace(/\s+/g,' '))) : null;});
  const cabec = await pg.evaluate(()=>{const t=document.getElementById('formTabla:tablaComparativoPlanCuota');
