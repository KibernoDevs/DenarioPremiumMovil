...
// 🔴 Playwright vive en el repo de QA, y esta carpeta puede estar FUERA de él
//    (hoy: OneDrive/Documentos/kiberno/documentacion). Por eso no se resuelve con
//    una ruta relativa fija: se buscan las ubicaciones conocidas y se usa la
//    primera que exista. Si mañana el repo cambia de sitio, se agrega aquí.
const CANDIDATOS_PW = [
  path.resolve(__dirname, '..', '..', 'automation', 'playwright', 'node_modules'),
  path.resolve(__dirname, '..', '..', 'qa-piloto-automatizacion', 'automation', 'playwright', 'node_modules'),
  'C:/Users/Personal/OneDrive/Documentos/kiberno/DenarioPremium/DenarioPremiunMovil/qa-piloto-automatizacion/automation/playwright/node_modules',
];
const PW = CANDIDATOS_PW.find(p => fs.existsSync(path.join(p, 'playwright')));
if (!PW) {
  console.log('ERR: no se encontró Playwright. Rutas probadas:');
  CANDIDATOS_PW.forEach(p => console.log('   ' + p));
  process.exit(1);
}
